...
REPEATS="${REPEATS:-3}"
SEED="${SEED:-7}"
OUT_DIR="${OUT_DIR:-compare_outputs/triplet_deferred_multi_release_stage_policy_gpt4mini_v1}"

TMP_CONFIG="$(mktemp /tmp/rcl_deferred_stage_policy.XXXXXX.json)"
trap 'rm -f "$TMP_CONFIG"' EXIT

cat >"$TMP_CONFIG" <<JSON
{
  "command": "compare-matrix",
  "script": "protocol_scripts/deferred_multi_release.json",
  "providers": [
  ...
  "args": {
    ...
    "delayed_mention_min_nonconclusion_items": 2,
    "delayed_mention_min_kind_diversity": 3,
...
python3 analyze_runs.py --log-dir "$OUT_DIR" --script protocol_scripts/deferred_multi_release.json
